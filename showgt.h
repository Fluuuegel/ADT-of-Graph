...
void writeGraph(Graph *g,FILE *fp){
	...
	for(int i = 0; i < g->vexnum; ++i){
		...
		if(g->kind == UDG || g->kind == UDN){
			while(a){
				if(Aarray[a->fromvex][a->adjvex] == 0){
					fprintf(fp,"{source: '%d', target: '%d', 'rela': '%d', type: 'resolved'},\n",a->fromvex,a->adjvex,a->weight);
					Aarray[a->fromvex][a->adjvex] = 1;
					Aarray[a->adjvex][a->fromvex] = 1;
				}	//防止无向图出现重边
				a = a->nextarc;
			}
		}else{
...
//与树的保存代码几乎完全一样，除了调用writeGraph()有差别
